...
app.get('/api/mercado', async (req, res) => {
  try {
    const { jogador1, jogador2, jogador3, dinheiro1, dinheiro2, dinheiro3, colecao1, colecao2, colecao3, porcentagem, comprado1, comprado2, comprado3 } = req.query;
...
const processPlayerImage = async (id, index, comprado, colecao) => {
  ...
  if (!fs.existsSync(playerPath)) {
    console.warn(`Imagem do jogador não encontrada: ${playerPath}`);
    return null;
  }

  let playerImage = sharp(playerPath).resize(350, 500);
  if (comprado === 'true') {
  ...
  let baseLayer = null;
  const colecaoAtual = colecoes[index];
  if (colecaoAtual && colecaoMap[colecaoAtual]) {
    const folderKey = colecaoMap[colecaoAtual];
...
//context.strokeText(formattedTextOriginal, 150, 50);
    ...
    const playerLayersArrays = await Promise.all(playerIds.map((id, index) => processPlayerImage(id, index, compradoFlags[index], colecoes[index])));
